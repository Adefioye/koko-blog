...
my guess is that SNMF as is trained here and in the paper is fairly wrong? The decomposed matrix, $Y$ has entry that shows how strongly
a feature represents a token instead of how strongly a feature represents a token in a sample? The paper trained on activation, $A \in \mathbb{R}^{mlp\_dim \times seq\_len}$
...
where we model tokens and their relationship in a sequence in the concept dataset. What we really want are features that encode concepts in the dataset
