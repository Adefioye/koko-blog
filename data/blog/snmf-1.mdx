...
SNMF decomposes activations using, $A \approx ZY$. $Z \in \mathbb{R}^{mlp\_dim \times k}, \quad Y \in \mathbb{R}_{\ge 0}^{k \times n}$.
Its objective is to minimize reconstruction loss using $\mathcal{L}(Z, Y) = \left\| A - Z Y \right\|_F^2$.

The following are the series of steps required to test out my research questions[1].
